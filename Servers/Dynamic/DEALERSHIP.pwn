...
SCRIPT BY BAYYY
DISCORD : zxbyy
BayyyStore : https://dc.gg/baystore
OPEN JASA DEVELOPER SAMP
	...
	foreach(new bid : Dealer)
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.5, DealerData[bid][dealerPosX], DealerData[bid][dealerPosY], DealerData[bid][dealerPosZ]))
		{
			if(DealerData[bid][dealerPrice] > pData[playerid][pMoney])
				return Error(playerid, "Not enough elektronik money, you can't afford this dealership.");
...
CMD:buypv(playerid, params[])
{
	...
	{
		if(IsPlayerInRangeOfPoint(playerid, 2.8, DealerData[dealerid][dealerPosX], DealerData[dealerid][dealerPosY], DealerData[dealerid][dealerPosZ]) && strcmp(DealerData[dealerid][dealerOwner], "-"))
		{
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_BUYDEALERCARS_CONFIRM)
	{
		...
		if(response)
		{
			...
			if(pData[playerid][pMoney] < cost)
			{
				Error(playerid, "Uang Elektronik anda tidak mencukupi.!");
				return 1;
			}
			new count = 0, limit = MAX_PLAYER_VEHICLE + pData[playerid][pVip];
			foreach(new ii : PVehicles)
